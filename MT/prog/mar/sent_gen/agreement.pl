...
close(TMP);
#### Telugu Part 
open(TMP,"$Data_Path/te/ki_karma_motion_verbs.txt") || die "Can't open ne_excptnl_verbs.txt for reading";
while(<TMP>) {
chomp;
$key = $_;
$EXCPT_KI{$key}=1;
...
foreach $w_ana (@wrd_ana){
  ...
  &add_ki();
  if($DEBUG) {&print_array("STDERR","HI Add ki",$wrd_fld,$morph_kaaraka_anal,$ana_fld_for_calling_gen_after_lwg,$ana_fld_for_calling_gen_after_lwg_karwari);}
  
  &add_wo();
  if($DEBUG) {&print_array("STDERR","Add wo",$wrd_fld,$morph_kaaraka_anal,$ana_fld_for_calling_gen_after_lwg,$ana_fld_for_calling_gen_after_lwg_karwari);}
